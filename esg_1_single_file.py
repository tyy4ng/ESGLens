...
# -*- coding: utf-8 -*-
"""
Process a single ESG/sustainability report PDF and extract structured information
using an LLM (GPT-4o-mini).

This script is the single-file counterpart of esg_1_multi-files.py. It is
useful for testing or analyzing one report interactively before running the
full batch pipeline.

Workflow:
    1. Parse the PDF and build a FAISS vector store (via ESGReport).
    2. Retrieve the most relevant text chunks for each ESG query.
    3. Call the LLM to extract content, tone, and numerical data in JSON format.
    4. Map chunk indices back to actual PDF page numbers.
    5. Save three CSV files: summary_context, summary_tone, summary_numeric.

Output structure (under SUMMARY_DIR/<report_name>/):
...
# =============================================================================
PDF_PATH             = './esg_report/NASDAQ_AAPL_2022.pdf'  # path to the target PDF
